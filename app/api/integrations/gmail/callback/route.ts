...
import { exchangeCodeForGmailTokens, getGmailUserProfile } from '@/lib/integrations/gmail'
  ...
  try {
    ...
    if (error) {
      return NextResponse.redirect(
        `${baseUrl}/settings?tab=integrations&error=gmail_auth_failed`
      )
    }
    ...
    return NextResponse.redirect(
      `${baseUrl}/oauth-success?provider=Gmail`
    )
  } catch (error) {
